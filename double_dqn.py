...
class PrioritizedReplayBuffer:
    def __init__(self, buffer_size, observation_space, action_space, device, alpha=0.6, beta=0.4):
    ...
    def sample(self, batch_size):
        ...
        return Batch(
            ...
            torch.as_tensor(weights).float().to(self.device),
            indices
        )
...
class DuelingQNetwork(nn.Module):
    """Dueling Q-Network architecture with tensor dimension safety"""
    def __init__(self, observation_space_shape, action_space_n):
        super().__init__()
        input_size = np.array(observation_space_shape).prod()
...
if __name__ == "__main__":
    ...
    # Create networks - using Dueling architecture for better performance
    q_network = DuelingQNetwork(env.observation_space.shape, env.action_space.n).to(device)
    optimizer = optim.Adam(q_network.parameters(), lr=args.learning_rate)
    target_network = DuelingQNetwork(env.observation_space.shape, env.action_space.n).to(device)
    ...
    rb = PrioritizedReplayBuffer(
        ...
        beta=0.4    # Start with low importance sampling correction
    )
    ...
    # Print start message
    print(f"Starting training on Enhanced Maize environment with Dueling DQN")
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    ...
                    next_q_values_target = target_network(data.next_observations)
                ...
                # Calculate loss with prioritized replay weights
                td_errors = td_target - current_q_values
                loss = (td_errors.pow(2) * data.weights).mean()
                
                # Update priorities in buffer
                with torch.no_grad():
                    new_priorities = np.abs(td_errors.cpu().numpy()) + 1e-6  # Small constant for stability
