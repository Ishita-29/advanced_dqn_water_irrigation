...
class EpisodeStats:
    ...
    # def plot_stats(self):
    #     # Set up larger plots with better styling
    ...
    #     # Plot rewards with smoothing and confidence intervals
    #     plt.figure(figsize=(12, 7))
    #     episodes = range(1, self.episode_count + 1)
        
    #     # Raw rewards (with alpha for better visibility)
    #     plt.plot(episodes, self.episode_rewards, alpha=0.3, color='royalblue', label='Raw Rewards')
        
    #     # Smoothed rewards
    #     if len(self.episode_rewards) >= 50:
    #         window_size = 50
    #         smoothed = smooth_curve(self.episode_rewards, window_size)
    #         # Plot from window_size/2 to account for the centered smoothing
    #         smooth_x = np.arange(window_size/2, len(self.episode_rewards) - window_size/2 + 1)
    #         plt.plot(smooth_x, smoothed, 
    #                  color='darkblue', linewidth=2.5, 
    #                  label=f'Smoothed (window={window_size})')
            
    #         # Compute rolling std and add confidence bands
    #         rolling_std = np.array([np.std(self.episode_rewards[max(0, i-window_size):i+1]) 
    #                                for i in range(window_size, len(self.episode_rewards))])
    #         plt.fill_between(smooth_x, 
    ...
    #     plt.title('Episode Rewards Over Time', fontsize=16, fontweight='bold')
    #     plt.grid(True, alpha=0.3)
    #     plt.legend(fontsize=12)
    #     plt.tight_layout()
    #     plt.savefig(os.path.join(plots_dir, 'rewards.png'), dpi=300)
    #     plt.close()
        
    #     # Plot yields with smoothing
    #     plt.figure(figsize=(12, 7))
    #     plt.plot(episodes, self.episode_yields, alpha=0.4, color='forestgreen', label='Yields')
        
    #     if len(self.episode_yields) >= 50:
    #         window_size = 50
    #         smoothed = smooth_curve(self.episode_yields, window_size)
    #         smooth_x = np.arange(window_size/2, len(self.episode_yields) - window_size/2 + 1)
    #         plt.plot(smooth_x, smoothed, 
    #                  color='darkgreen', linewidth=2.5, 
    #                  label=f'Smoothed (window={window_size})')
            
    #         # Add confidence bands
    #         rolling_std = np.array([np.std(self.episode_yields[max(0, i-window_size):i+1]) 
    #                                for i in range(window_size, len(self.episode_yields))])
    #         plt.fill_between(smooth_x, 
    #                          smoothed - rolling_std, 
    #                          smoothed + rolling_std, 
    ...
    #     plt.xlabel('Episode', fontsize=14)
    #     plt.ylabel('Dry Yield (tonne/ha)', fontsize=14)
    #     plt.title('Crop Yields Over Time', fontsize=16, fontweight='bold')
    #     plt.grid(True, alpha=0.3)
    #     plt.legend(fontsize=12)
    #     plt.tight_layout()
    #     plt.savefig(os.path.join(plots_dir, 'yields.png'), dpi=300)
    #     plt.close()
        
    #     # Plot irrigation with smoothing
    #     plt.figure(figsize=(12, 7))
    #     plt.plot(episodes, self.episode_irrigation, alpha=0.4, color='steelblue', label='Irrigation')
        
    #     if len(self.episode_irrigation) >= 50:
    #         window_size = 50
    #         smoothed = smooth_curve(self.episode_irrigation, window_size)
    #         smooth_x = np.arange(window_size/2, len(self.episode_irrigation) - window_size/2 + 1)
    #         plt.plot(smooth_x, smoothed, 
    #                  color='navy', linewidth=2.5, 
    #                  label=f'Smoothed (window={window_size})')
            
    #         # Add confidence bands
    #         rolling_std = np.array([np.std(self.episode_irrigation[max(0, i-window_size):i+1]) 
    #                                for i in range(window_size, len(self.episode_irrigation))])
    #         plt.fill_between(smooth_x, 
    #                          smoothed - rolling_std, 
    #                          smoothed + rolling_std, 
    ...
    #     plt.xlabel('Episode', fontsize=14)
    #     plt.ylabel('Total Irrigation (mm)', fontsize=14)
    #     plt.title('Irrigation Applied Over Time', fontsize=16, fontweight='bold')
    #     plt.grid(True, alpha=0.3)
    #     plt.legend(fontsize=12)
    #     plt.tight_layout()
    #     plt.savefig(os.path.join(plots_dir, 'irrigation.png'), dpi=300)
    #     plt.close()
        
    #     # Plot Water Use Efficiency if available
    #     if self.episode_wue:
    #         plt.figure(figsize=(12, 7))
    #         plt.plot(episodes, self.episode_wue, alpha=0.4, color='purple', label='WUE')
            
    #         if len(self.episode_wue) >= 50:
    #             window_size = 50
    #             smoothed = smooth_curve(self.episode_wue, window_size)
    #             smooth_x = np.arange(window_size/2, len(self.episode_wue) - window_size/2 + 1)
    #             plt.plot(smooth_x, smoothed, 
    #                      color='darkmagenta', linewidth=2.5, 
    #                      label=f'Smoothed (window={window_size})')
                
    #             # Add confidence bands
    #             rolling_std = np.array([np.std(self.episode_wue[max(0, i-window_size):i+1]) 
    #                                    for i in range(window_size, len(self.episode_wue))])
    #             plt.fill_between(smooth_x, 
    #                              smoothed - rolling_std, 
    #                              smoothed + rolling_std, 
    ...
    #         plt.xlabel('Episode', fontsize=14)
    #         plt.ylabel('Water Use Efficiency (yield/irrigation)', fontsize=14)
    #         plt.title('Water Use Efficiency Over Time', fontsize=16, fontweight='bold')
    #         plt.grid(True, alpha=0.3)
    #         plt.legend(fontsize=12)
    #         plt.tight_layout()
    #         plt.savefig(os.path.join(plots_dir, 'water_use_efficiency.png'), dpi=300)
    #         plt.close()
